...
$imageAttr = new \classes\Image();

$imageAttr -> getImageData($image_url);
if(empty($imageAttr -> imageInfo)){
	return \classes\Response::error(404, array('error' => true, 'code' => 404, 'data' => 'Image not found'));
}

if(isset($_REQUEST['raw']) && $_REQUEST['raw'] == 1){
	
	echo \classes\Response::JSON(200, $imageAttr -> imageInfo);
	exit();

}

$html = $imageAttr -> getTemplate();

if(isset($_REQUEST['html']) && $_REQUEST['html'] == 1){

	$imageAttr -> imageInfo['html'] = htmlentities(htmlspecialchars($html));
	echo \classes\Response::JSON(200, $imageAttr -> imageInfo);
	exit();
	...
	?>
	<div id="gallery91_wrap" style="position: relative;top: 10px;left: 10px;width: 100%;height: 100%;">
		<link rel="stylesheet" type="text/css" href="<?php echo \Config::CssJSUrl; ?>css/style.css?v=3">
		<meta property="fb:page_id" content="132814090096744" />
		<meta property="fb:app_id" content="150997098245063"/>
	...
	<script type="text/javascript">
		var config = {};
		config['image_path_url'] = '<?php echo \Config::ImageViewURL; ?>';
		config['css_js_path'] = '<?php echo \Config::CssJSUrl; ?>';
		config['url_path'] = '<?php echo \Config::urlPath; ?>';
		config['images_path'] = '<?php echo \Config::imagesPath; ?>';
	</script>
	<script type="text/javascript" src="<?php echo \Config::CssJSUrl; ?>js/jquery.min.js"></script>
	<script type="text/javascript" src="<?php echo \Config::CssJSUrl; ?>js/jquery.event.swipe.js"></script>
	<script type="text/javascript">
